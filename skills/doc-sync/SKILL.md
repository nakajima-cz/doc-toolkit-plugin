...
### Step 3.5:【確認②】更新内容の事前予測提示とユーザー承認

ドキュメントの更新を開始する前に、各ドキュメントで何が変わるかの**予測概要**をユーザーに提示し、承認を得る。

> **注意**: ここでの提示内容は、対応エージェントを起動する前の時点での予測です。
> ソースコードの変更内容を読み取って変更箇所を推定したものであり、実際の差分ではありません。
> 実際の更新は承認後に対応エージェントが行います。

```
## 更新予定のドキュメント（予測）

以下のドキュメントを更新します。変更の概要を確認してください。
...
**ユーザーが承認するまで、対応エージェント（doc生成エージェント）の起動を行わない。**
